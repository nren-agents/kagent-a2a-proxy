...
- In-progress tool calls (`kagent_type=function_call`) → Thinking pane as a blockquote `> 🔧 **{name}**` + optional `> \`{compact args}\`` (`_format_tool_args`, truncated to `_MAX_ARGS_LEN`), via the shared `_tool_call_chunks`. Tool results (`kagent_type=function_response`) are **dropped**.
