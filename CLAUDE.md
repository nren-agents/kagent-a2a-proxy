...
- kagent re-sends each answer burst three ways: as `working` **partial** deltas (`kagent_adk_partial=true`), once as a **non-partial aggregate** (`kagent_adk_partial=false`) — which for a *narration* burst is bundled with the `function_call` it triggered — then the final burst again as an **`artifact-update`**. **`settings.narration_mode` (config) selects how `_working_chunks` renders this**, dispatching to one of two paths:
  - **`stream`**: token-stream the partial deltas — narration *and* answer-in-progress alike — verbatim into **`reasoning_content`** (Thinking pane); skip the aggregate (`is_partial() is False`). Because nothing streams to `content`, `agent_runner` **keeps** the trailing artifact and renders it as the main-pane answer (delivered at once, not token-streamed). Keeps the live-streaming feel but moves the noise out of the visible reply.
  ...
  - Non-ADK executors (`is_partial() is None`, no aggregate): in `deemphasize` their text goes directly to `content` (the artifact is the dropped duplicate); in `stream` their text goes to `reasoning_content` and the artifact becomes the `content` answer.
...
**Streaming vs. blocking.** Both paths share `agent_runner.translate_stream` (stream → `parse_sse_line` → `event_to_chunks` → drop the duplicate artifact + a consecutive-identical-reasoning safety net). `main.py` yields chunks straight to SSE for `stream=true`, wrapped in `_with_heartbeat`: after `PROXY_SSE_HEARTBEAT_INTERVAL` (default 20) seconds of idle it emits an SSE comment (`: ping`) so idle-read timeouts on the client path (LibreChat's undici ~300s bodyTimeout, LB idle timers) don't sever long-quiet agent streams; `0` disables. For blocking responses and all MCP tool calls, `agent_runner.collect_agent_response` drains the same pipeline and concatenates `delta.content` into the final string; `reasoning_content` deltas instead fire an optional `on_progress` callback (MCP tools forward these as progress notifications).
...
**Human-in-the-loop (`hitl.py`).** When `PROXY_HITL_SECRET` is set, an `input-required` tool-approval prompt carries an HMAC-signed, render-invisible marker (zero-width chars) encoding the paused task's `taskId`/`contextId`. On the next request, `main._select_chunks` recovers it from the assistant history (`hitl.extract_pending`), classifies the user reply (`hitl.classify_decision`), and resumes via `kagent_client.resume_stream` (a decision `DataPart`) instead of a fresh prompt. Stateless — no store, works across replicas (LibreChat resends assistant content verbatim). The resume wire shape matches kagent's documented HITL protocol (`docs/architecture/human-in-the-loop.md` in kagent): a `DataPart` `{"decision_type": "approve"|"reject"}` with `taskId`/`contextId` on the message; a single uniform decision applies to **all** pending confirmations (kagent's `decision_type: "batch"` exists for mixed per-tool decisions but isn't needed here).

**Session continuity (`hitl.py`, same marker).** OpenAI chat completions are stateless, so by default the proxy minted a fresh `sessionId` per request and kagent saw every POST as a new chat. When `PROXY_HITL_SECRET` is set, the `completed` event embeds a **context-only** marker (`encode_marker("", contextId, …)` via `translator._session_marker`) carrying the kagent conversation's `contextId`. On the next turn `main._select_chunks` recovers it (`hitl.extract_context`, scanning assistant history newest-first) and — when there's no pending HITL approval — **continues** that conversation: it sends **only the newest turn** (`_messages_after_last_assistant`) under that `contextId` via `kagent_client._build_continue_payload` (message-level `contextId`, no `sessionId` — mirrors the resume shape; per A2A a `contextId` without `taskId` opens a new task in the existing context). kagent rebuilds prior history from its server-side session store; the proxy stays stateless. Signing is load-bearing: it stops a client forging a `contextId` to hijack another conversation's session. With the secret unset, continuity is off and every turn is a fresh full-history replay (the original behavior). MCP tool output is one-shot, so `agent_runner.collect_agent_response` strips the marker (`hitl.strip_marker`). **Continuity tombstone:** terminal-failure notices (`failed`/`canceled`/`rejected`/`auth-required`) and mid-stream JSON-RPC error notices embed a tombstone marker (`hitl.encode_tombstone`) — a killed/aborted run can leave a dangling `function_call` in kagent's session, which the LLM then rejects on every continued turn (Anthropic: "`tool_use` ids without `tool_result`"). `extract_context` stops at the newest decisive marker (tombstone → fresh session; context marker → continue), so one errored turn breaks the chain instead of endlessly reopening a poisoned context; a later completed turn re-establishes it. NOTE: this assumes kagent honors the echoed `contextId` and persists session history server-side (controller session API / task store, `local=False`) — some ADK A2A setups fall back to an in-memory session service, so validate against a live kagent.
